...
%         ylim([0 10])
        ...
        if isempty(saclocs)
            % if no saccades, both non preferred targets 
    ...
    %% now that all the matricies are collected, plot the heatmaps     
     ...
     hbins = linspace(-xcent*2, xcent*2, xdiv);
     vbins = linspace(-ycent*2, ycent*2, ydiv);
     
     for col = 1:ydiv - 1
         for row = 1:xdiv - 1
             
             indFlashPref = find(prefXlocs > hbins(row) & prefXlocs < hbins(row+1) & ...
                 prefYlocs > vbins(col) & prefYlocs < vbins(row+1));
             ...
             indFlashNonPref = find(nonprefXlocs > hbins(row) & nonprefXlocs < hbins(row+1) & ...
                 nonprefYlocs > vbins(col) & nonprefYlocs < vbins(row+1));
